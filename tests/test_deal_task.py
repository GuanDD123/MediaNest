from pathlib import Path

from media_nest.service.deal_task import DealTask
from media_nest.models import TaskInfo
from tests.tool.run_collect_info import run_collect_info
from tests.fake.fake_folder_file import create_folder_file

ROOT = Path(__file__).parents[1] / "Untitled Folder"


class FakeRepository:
    def __init__(self):
        self.task_info_list = [
            TaskInfo(
                id=1,
                type_="video",
                path=ROOT / "视频/video_3.mp4",
                dev=66312,
                ino=2693,
                duration_ms_flag=True,
                width_height_flag=True,
                hls_flag=True,
                thumb_flag=False,
            ),
            TaskInfo(
                id=2,
                type_="video",
                path=ROOT / "视频/video_0.mp4",
                dev=66312,
                ino=2690,
                duration_ms_flag=True,
                width_height_flag=True,
                hls_flag=True,
                thumb_flag=False,
            ),
            TaskInfo(
                id=3,
                type_="video",
                path=ROOT / "视频/video_5.mp4",
                dev=66312,
                ino=2695,
                duration_ms_flag=True,
                width_height_flag=True,
                hls_flag=True,
                thumb_flag=False,
            ),
            TaskInfo(
                id=4,
                type_="video",
                path=ROOT / "视频/video_2.mp4",
                dev=66312,
                ino=2692,
                duration_ms_flag=True,
                width_height_flag=True,
                hls_flag=True,
                thumb_flag=False,
            ),
            TaskInfo(
                id=5,
                type_="video",
                path=ROOT / "视频/video_4.mp4",
                dev=66312,
                ino=2694,
                duration_ms_flag=True,
                width_height_flag=True,
                hls_flag=True,
                thumb_flag=False,
            ),
            TaskInfo(
                id=6,
                type_="video",
                path=ROOT / "视频/video_6.mp4",
                dev=66312,
                ino=2696,
                duration_ms_flag=True,
                width_height_flag=True,
                hls_flag=True,
                thumb_flag=False,
            ),
            TaskInfo(
                id=7,
                type_="video",
                path=ROOT / "视频/video_1.mp4",
                dev=66312,
                ino=2691,
                duration_ms_flag=True,
                width_height_flag=True,
                hls_flag=True,
                thumb_flag=False,
            ),
            TaskInfo(
                id=8,
                type_="image",
                path=ROOT / "图片/test.webp",
                dev=66312,
                ino=2689,
                duration_ms_flag=False,
                width_height_flag=True,
                hls_flag=False,
                thumb_flag=True,
            ),
            TaskInfo(
                id=9,
                type_="image",
                path=ROOT / "图片/test.png",
                dev=66312,
                ino=2688,
                duration_ms_flag=False,
                width_height_flag=True,
                hls_flag=False,
                thumb_flag=True,
            ),
            TaskInfo(
                id=10,
                type_="image",
                path=ROOT / "图片/风景/img_7.jpg",
                dev=66312,
                ino=2684,
                duration_ms_flag=False,
                width_height_flag=True,
                hls_flag=False,
                thumb_flag=True,
            ),
            TaskInfo(
                id=11,
                type_="image",
                path=ROOT / "图片/风景/img_9.jpg",
                dev=66312,
                ino=2686,
                duration_ms_flag=False,
                width_height_flag=True,
                hls_flag=False,
                thumb_flag=True,
            ),
            TaskInfo(
                id=12,
                type_="image",
                path=ROOT / "图片/风景/img_1.jpg",
                dev=66312,
                ino=2678,
                duration_ms_flag=False,
                width_height_flag=True,
                hls_flag=False,
                thumb_flag=True,
            ),
            TaskInfo(
                id=13,
                type_="image",
                path=ROOT / "图片/风景/img_10.jpg",
                dev=66312,
                ino=2687,
                duration_ms_flag=False,
                width_height_flag=True,
                hls_flag=False,
                thumb_flag=True,
            ),
            TaskInfo(
                id=14,
                type_="image",
                path=ROOT / "图片/风景/img_3.jpg",
                dev=66312,
                ino=2680,
                duration_ms_flag=False,
                width_height_flag=True,
                hls_flag=False,
                thumb_flag=True,
            ),
            TaskInfo(
                id=15,
                type_="image",
                path=ROOT / "图片/风景/img_6.jpg",
                dev=66312,
                ino=2683,
                duration_ms_flag=False,
                width_height_flag=True,
                hls_flag=False,
                thumb_flag=True,
            ),
            TaskInfo(
                id=16,
                type_="image",
                path=ROOT / "图片/风景/img_4.jpg",
                dev=66312,
                ino=2681,
                duration_ms_flag=False,
                width_height_flag=True,
                hls_flag=False,
                thumb_flag=True,
            ),
            TaskInfo(
                id=17,
                type_="image",
                path=ROOT / "图片/风景/img_2.jpg",
                dev=66312,
                ino=2679,
                duration_ms_flag=False,
                width_height_flag=True,
                hls_flag=False,
                thumb_flag=True,
            ),
            TaskInfo(
                id=18,
                type_="image",
                path=ROOT / "图片/风景/img_8.jpg",
                dev=66312,
                ino=2685,
                duration_ms_flag=False,
                width_height_flag=True,
                hls_flag=False,
                thumb_flag=True,
            ),
            TaskInfo(
                id=19,
                type_="image",
                path=Path(ROOT / "图片/风景/img_0.jpg"),
                dev=66312,
                ino=2677,
                duration_ms_flag=False,
                width_height_flag=True,
                hls_flag=False,
                thumb_flag=True,
            ),
            TaskInfo(
                id=20,
                type_="image",
                path=Path(ROOT / "图片/风景/img_5.jpg"),
                dev=66312,
                ino=2682,
                duration_ms_flag=False,
                width_height_flag=True,
                hls_flag=False,
                thumb_flag=True,
            ),
        ]
        self.segment_insert_list = []
        self.video_update_list = []
        self.image_update_list = []

    def task_select_all(self):
        return self.task_info_list

    def task_delete_all(self):
        self.task_info_list = []

    def node_update_many_width_height_by_dev_ino(self, image_update_list):
        self.image_update_list.append(image_update_list)

    def node_select_id_join_task_dev_ino_by_hlsflag(self):
        return {f"{task.dev}_{task.ino}": task.id for task in self.task_info_list}

    def segment_delete_in_video_id(self, ids):
        pass

    def node_update_many_width_height_duration_by_dev_ino(self, video_update_list):
        self.video_update_list.append(video_update_list)

    def segment_insert_many(self, segment_insert_list):
        self.segment_insert_list.append(segment_insert_list)


def test_deal_task():
    repository = FakeRepository()
    create_folder_file(video_num=7, image_num=11)

    do = "deal task"
    run_collect_info(DealTask(repository).run, (), do)
    assert repository.task_select_all() == []
    print(repository.segment_insert_list)
    assert repository.segment_insert_list
    assert repository.image_update_list
    assert repository.video_update_list
