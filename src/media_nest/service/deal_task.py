...
class DealTask:
    ...
    def _process_video(self, task: TaskInfo, dev_ino_video_id_dict):
        try:
            ...
            if task.duration_ms_flag:
                ...
                result[0] = ((task.dev, task.ino), int(stream['width']), int(stream['height']),
    ...
    def _build_hls(self, video_path: Path, hls_dir: Path):
        ...
        if HLS_MODE == 'fMP4':
            cmd = ['ffmpeg', '-hide_banner', '-loglevel', 'error', '-i', str(video_path), '-c', 'copy', '-f', 'hls',
                   '-hls_time', '4', '-hls_list_size', '0', '-hls_playlist_type', 'vod', '-hls_segment_type', 'fmp4',
                   '-hls_flags', 'independent_segments', '-hls_fmp4_init_filename', 'init.mp4',
                   '-hls_segment_filename', str(hls_dir / 'seg_%05d.m4s'), str(hls_dir / 'index.m3u8')]
        else:
            cmd = ['ffmpeg', '-i', str(video_path), '-c', 'copy', '-f', 'hls', '-hls_time', '4', '-hls_list_size', '0',
