...
def model_to_row(info: FolderInfo | VideoInfo | ImageInfo | RootInfo | TaskInfo, table: str) -> tuple:
    if table == 'root':
...
def row_to_model(row: tuple, table: str) -> RootInfo | TaskInfo | FolderInfo | VideoInfo | ImageInfo:
    if table == 'root':
        root = Root(*row)
